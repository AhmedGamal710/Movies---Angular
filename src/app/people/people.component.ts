...
export class PeopleComponent implements OnInit {
  ...
  activatePageNum(): any {
    ...
    return parseInt(this._ActivatedRoute.snapshot.params['page']) <= 0
      ? (this._Router.navigate(['/person/1']), 1)
  ...
  getMovies(number: number): void {
    setTimeout(() => {
      this.moviesSubscrip = this._MoviesapiService
        ...
        .subscribe({
          next: (response) => {
            this.trendingPer = response.results;
          ...
          complete: () => {
            this._MoviesapiService.shardPageNumberPer.next(this.activatePageNum()); // send activate number to shard movie service
          },
          error: (error) => {
